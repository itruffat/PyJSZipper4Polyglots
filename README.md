...
Solving the edge cases of all the other languages is Ruby is almost trivial, a handful of strings replacement  is enough 
to do the trick. However, trying to avoid the edge-cases generated by Ruby is another can of worms, one we will take a
...
Ruby is the last language our template ignore, so we are free to use any tool the language can provide. However, due to 
the nature of it's multiline comments/strings , our options are limited. All of them have their own drawback and can 
...
This option is outright unusable for us, as it would not allow us to use `)` on any of our other code-bases. This is 
of course, a drawback I'd not be willing to take.
...
This one is better, but not being able to use `=end` on any of our other code-bases can also be an issue. There are 
cases where `=end` might be inside a string, case in which we could use the replacement trick we used before. But there
are also cases where this syntax is valid.

    // if you change `end` for end_x,
    // you'd have to rename the var to end_x_of_time in line 1 as well

    end_of_the_line=12
    stop_before=end_of_the_line - 1


      while current_position!==end_of_the_line:
                 print(" ==end is still far away== ")
                 keep_going()
...
This option is slightly better, as now it's implicit we have to worry about the cases where the variable does not have
`=` at the end. We can also select longer and weirder variables names, to avoid any collision with the other languages.
